...
Folders covered:
  ...
  - lessons/   → sorted by date descending; grouped by primary subsystem
...
def render_lessons_index(folder_name: str, folder: Path) -> str:
    ...
    for p, text, fm in load_md_files(folder, recursive=True):
        ...
        sub_match = re.search(r"^subsystems:\s*\[([^\]]*)\]", text, re.MULTILINE)
        if sub_match:
            subs = [s.strip().strip("'\"") for s in sub_match.group(1).split(",") if s.strip()]
        else:
            subs = []
        primary = subs[0] if subs else "(no subsystem)"
    ...
    lines = [
        f"# {folder_name.capitalize()} — index",
        "",
        f"_Auto-generated by `plugins/lore/scripts/regenerate_indices.py`. Grouped by primary subsystem; newest first within each group. Edit source files, not this index._",
        "",
        f"**Total active:** {len(active)}  ·  **Superseded:** {len(superseded)}",
        "",
        "## What this is",
        "",
        "Lessons are mistakes (process, judgment, coordination, technical) with concrete prevention checks. Surfaced at SessionStart for matched subsystems alongside dead-ends. Consult during /planning, /brainstorm, /council-session.",
        "",
    ]
